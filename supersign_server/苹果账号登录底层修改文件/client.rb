...
module Spaceship
  ...
  class Client
    ...
    def self.login(user = nil, password = nil,callbk = nil)
      instance = self.new
      if instance.login(user, password,callbk)
        instance
      else
    ...
    def self.startVerify(codeNum = nil)
      startVerification(codeNum)
    end
    ...
    #
    # @return (Spaceship::Client) The client the login method was called for
    def login(user = nil, password = nil,callbk = nil)
      ...
      @password = password
      begin
        do_login(user, password,callbk) # calls `send_login_request` in sub class (which then will redirect back here to `send_shared_login_request`, below)
    ...
    def send_shared_login_request(user, password,callbk = nil)
      ...
      when 409
        ...
        handle_two_step_or_factor(response,callbk)
    ...
    def do_login(user, password,callbk)
      @loggedin = false
      ret = send_login_request(user, password,callbk) # different in subclasses
      @loggedin = true
      ret
    end
