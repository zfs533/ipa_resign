...
module Spaceship
  class Client
    def handle_two_step_or_factor(response,callbk = nil)
      ...
      # get authentication options请求验证码
    ...
    def handle_two_factor(response, depth = 0,callbk = nil)
      if depth == 0
      ...
      puts env_2fa_sms_default_phone_number
      ...
      begin
        ...
        if ex.to_s.include?("verification code") # to have a nicer output
          puts("Error: Incorrect verification code")
          depth += 1
          return handle_two_factor(response, depth,callbk)
        end
    ...
    def self.startVerification(codeNum = nil)
    ...
    def ask_for_2fa_code(text,callbk = nil)
      if callbk
        verificationNum = callbk.call()
        puts verificationNum
        return verificationNum
      end
      puts "callback func is nil!"
      ask(text)
    end
